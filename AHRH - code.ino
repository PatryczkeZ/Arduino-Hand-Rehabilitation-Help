...
{
  delay(200); //Konieczne żeby po wejściu do opcji nie zmieniała się wartość
  while(a == 1){
...
void exercise() //Funkcja z kodem ćwiczenia
{
  ...
  if(difficulty == 2){                 //Rozmiar kwadratu zależny od zmiennej difficulty
    rectSize = 10;
    rectPossitionChange = 0;
  }
  if(difficulty == 3){
  ...
  remClockTime = millis();    //Czas wejścia w ćwiczenie, potrzebny do odliczania czasu
  while(a == 1){
    int16_t adc0;
    float volts0;
    ...
    if (difTime >= 100){
        remTime = millis();         //Opóźnienie z millisem
    }
...
void setup(void)
{
  ...
  if (!ads.begin()) {
    Serial.println("Failed to initialize ADS."); //Warunek, który w przypadku problemów z inicjacją ADS wyświetla informację w serial monitorze
    while (1);
  }

  if(!display.begin(SSD1306_SWITCHCAPVCC, SCREEN_ADDRESS)) {
    Serial.println(F("SSD1306 allocation failed"));          //Warunek, który w przypadku błędu związanego z ekranem wyświetla informację w serial monitorze
    for(;;);
  }
...
void loop(void)  //Główna pętla programu, w której mamy menu 
{
  display.clearDisplay();
  display.setTextColor(1);
